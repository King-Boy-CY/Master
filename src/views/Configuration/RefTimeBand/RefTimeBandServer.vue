...
export default class RefTimeBandServer extends Vue {
    @Prop() addEdit: any;
    private unsetBodyPadding:boolean = true; //是否清除
    ...
    public actions() {
        ...
        const arr: any = [];
        this.index++;
        // let begin = moment().endOf('m');
        // let end = moment().set('year',2099).endOf('y');
        this.index + 1;
        let options: object = [{
                type: 'select',
        ...
        const actions = new Map([
            ...
            [5, ["picker",`beginDateTime${this.index}`,"","",""]],
            [6, ["picker",`endDateTime${this.index}`,"","",""]]
        ]);
        for (let index = 1; index < actions.size + 1; index++) {
            ...
            arr.push({
                ...
                decorator: action[1],
                options: action[2],
                initialValue: action[3] || undefined,
    ...
    private selectchange(inputdata: any, name: any, u, key: number, index: number) {
        this.$nextTick(() => {
            ...
            // console.log(notempty(weekdayFromArr));
            // console.log(notempty(weekdayToArr));
            // console.log('isHasDay', isHasDay);
            // console.log('isHasWeek', isHasWeek);
