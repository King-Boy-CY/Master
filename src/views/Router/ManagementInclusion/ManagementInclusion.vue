<template>
  <div id="managementInclusion">
    ...
    <!-- table -->
    <div>
      <simple-table :columns="copyColumns" :propTotal="pagination.total" :data="data" :border="border" ref="simpleTable" :isAlphabet="isAlphabet" @handletablechange="tableChange" @selectedRows="onSelectChange" @selectedRowKeys="onSelectKeys"></simple-table>
    </div>
    <!-- add Modal -->
    <div class="add-model">
      <model
...
export default class ManagementInclusion extends Vue {
  private form: any
  // 控制form隐藏显示
  ...
  //标识哪个页面调用formGroup
  private identifying :any = {name:'inclusion',num:0}
  ...
  private columns: IColumns[] = inclusionColumns
  ...
  // topFrom
  private topFromOptions: IFormoptions[] = inclusionFrom

  // add model formOptions
  private addFormOptions: IFormoptions[] = inclusionAddFrom

  // edit model formOptions
  private editFormOptions: IFormoptions[] = inclusionEditFrom
  ...
  //search submit
  private url: string = "/route/inclusion/query"
  ...
  private export(){
    ...
    let exparams:any = {
      params:{
          ...
          account: values.account ? values.account : "",
          status: values.status ? values.status : "",
          remark: values.account ? values.remark : "",
  ...
  //add弹框  点击save
  private addUrl: string = "/route/inclusion/add"
  private addSave () {
  ...
  //edit弹框  点击save
  private editUrl: string = "/route/inclusion/update"
  private editSave () {
  ...
  private modalOk(eventType: string): void {
    ...
    if (eventType === 'edit') {
      ...
      const uploadUrl = `/route/inclusion/import`
...
<style scoped>
#managementInclusion >>> .ant-col-8{
  padding-left: 3% !important;
}
#managementInclusion >>> .ant-col-12{
  padding-left: 6% !important;
}
#managementInclusion >>> .ant-col-5{
  padding-left: 4px !important;
}
#inclusionAdd >>> .ant-col-20{
  padding-left: 1.8%;
}
#inclusionEdit >>> .ant-col-20{
  padding-left: 1.8%;
}
</style>
