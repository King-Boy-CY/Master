<template>
  <div id="routeProduct">
    ...
    <!-- add Modal -->
    <div class="add-model">
      <model title="Add benchmark for route product" v-model="visible" width="1200px">
        <template slot="model-body">
          <div id="productAdd">
            <form-group ref="routeProductAdd" :formoptions="addFormOptions" @selectChange="selectChange" @monthPickerChange="addMonthPickerChange" @inputChange="addInputChange"></form-group>
          </div>
        </template>
    ...
    <div class="edit-model">
      <model title="Edit benchmark for route product " v-model="editVisible" width="1200px">
        <template slot="model-body">
          <div id="routeClassEdit">
            <form-group ref="routeProductEdit" :formoptions="editFormOptions" @selectChange="selectChange" @monthPickerChange="editMonthPickerChange" @inputChange="editInputChange"></form-group>
          </div>
        </template>
        <template slot="model-footer">
          <a-row>
...
export default class RoutingProduct extends Vue {

  private identifying :any = {name:'routeProduct',num:31}
  private form: any
  // 控制form隐藏显示
  ...
  private columns: IColumns[] = routeProductColumns
  private copyColumns: IColumns[] = routeProductColumns
  private data: any = []
  ...
  // topFrom
  private topFromOptions: IFormoptions[] = routeProductFrom

  // add model formOptions
  private addFormOptions: IFormoptions[] = routeProductAddFrom

  // edit model formOptions
  private editFormOptions: IFormoptions[] = routeProductEditFrom
  ...
  //查询url
  private queryUrl: string = `/route/route-product/query`
  ...
  private beforeUpload(fileList: string[]): void {
    if (fileList.length > 0) {
      const title =  `Are you going to upload ${fileList.length} file?`
      this.confirmModal("upload", title,{okText:'Proceed'} )
    }
  }

  private delete(): void {
    postDelete(this.selectedId).then(res => {
      if (res.code === Status.Ok) {
   ...
   */
  private search() {
    const url = `/route/route-product/query`
    let params = {
      params: this.formData,
      pageSize: this.pagination.pageSize,
  ...
  private export(){
    ...
    axios({
      url:'/route/route-product/export',
      data:exparams,
      method:'post',
      responseType: 'blob',
      }).then((data)=>{
  ...
  private addAndEditSave(type: string): void {
    let formrefs: any = type === 'edit' ? (this.$refs['routeProductEdit'] as any) : (this.$refs['routeProductAdd'] as any)
  ...
  private addSave(params) {
    let re=/^\d+$|^\d*\.\d+$/g
    if(params.asrMinimum !== '' && params.asrMinimum !== undefined){
      if (!re.test(params.asrMinimum)) {
        this.$message.error('You can only enter Numbers and decimals')
      }else{
        postComercialAdd(params, '/route/route-product/add').then(res => {
        if (res.code === Status.Ok) {
          this.visible = false
          this.data = [res.data]
          this.pagination['pageNum'] = Pagination.pageNum
  ...
  private modalOk(eventType: string): void {
    ...
    if (eventType === 'edit') {
      ...
      const uploadUrl = `/route/route-product/import`
  ...
  private editSave(params) {
    let re=/^\d+$|^\d*\.\d+$/g
    console.log(params.asrMinimum)
    if(params.asrMinimum !== '' && params.asrMinimum !== undefined){
      if (!re.test(params.asrMinimum)) {
        this.$message.warning('You can only enter Numbers and decimals')
      }else{
      putComercialEdit(params, '/route/route-product/update').then(res => {
        if (res.code === Status.Ok) {
   ...
   */
  public handleReset() {
    const formRef = this.$refs['formgroup'] as any
    formRef.resetFields()
  }
  ...
  private addMonthPickerChange(e, val, decorator) {
    ...
    if (compareDate(beginDate, endDate) === false) {
      const addFormRefs = this.$refs['routeProductAdd'] as any
  ...
  private editMonthPickerChange(e, val, decorator) {
    ...
    const endDate: string = val['endDate'] === null || val['endDate'] === undefined ? '' : moment(val['endDate']).format('YYYY-MM-DD')
    if (compareDate(beginDate, endDate) === false) {
      const editFormRefs = this.$refs['routeProductEdit'] as any
      editFormRefs.resetFields([decorator])
      this.errorModal(decorator, warn)
    }
  }
  private addInputChange(e: any, values: any,decorator: string): void {
    this.fieldsVerify(e, 'routeProductAdd', values, decorator)
  }
  private editInputChange(e: any, values: any,decorator: string): void {
    this.fieldsVerify(e, 'routeProductEdit', values, decorator)
  }
...
<style scoped>
#routeProduct >>> .ant-col-8 {
  padding-left: 3% !important;
}
#routeProduct >>> .ant-col-12 {
  padding-left: 6% !important;
}
#routeProduct >>> .ant-col-5 {
